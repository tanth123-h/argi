...
class FarmMapScreen extends StatefulWidget {
  ...
  const FarmMapScreen({Key? key, required this.farmId}) : super(key: key);
...
class _FarmMapScreenState extends State<FarmMapScreen> {
  ...
  Future<void> _loadFarm() async {
    try {
      ...
      // If farm has polygon data, load it
  ...
  void _loadExistingPolygon(List<Map<String, double>> coordinates) {
    ...
    if (points.isNotEmpty) {
      _mapController?.animateCamera(
        CameraUpdate.newLatLngBounds(_calculateBounds(points), 50.0),
      );
    }
  }
        ...
        children: [
          GoogleMap(
            ...
            initialCameraPosition: const CameraPosition(
              target: LatLng(14.5995, 120.9842), // Philippines default
              zoom: 15,
            ),
            polygons: _polygons,
            markers: _markers,
